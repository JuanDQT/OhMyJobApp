            ...
            public void onResponse(String response) {
                ...
                try {
                    ...
                    if (img_path.equals("null") || img_path.isEmpty()) {
                        Picasso.with(getApplicationContext()).load(R.drawable.new_user).transform(new BlurTransformation(getApplicationContext(),25)).into(iv_fondo);
                        Picasso.with(getApplicationContext()).load(R.drawable.new_user).transform(new CircleTransform()).into(civ_pic);

                    }else {
                        Log.d("USERPROFILE", "else image");
                        Picasso.with(getApplicationContext()).load(img_path).memoryPolicy(MemoryPolicy.NO_CACHE).memoryPolicy(MemoryPolicy.NO_STORE).transform(new BlurTransformation(getApplicationContext(),25)).into(iv_fondo);
                        Picasso.with(getApplicationContext()).load(img_path).memoryPolicy(MemoryPolicy.NO_CACHE).memoryPolicy(MemoryPolicy.NO_STORE).transform(new CircleTransform()).into(civ_pic);
                    }
